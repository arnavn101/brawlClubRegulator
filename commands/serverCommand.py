...
class ServerCommand(BaseCommand):
    ...
    async def handle(self, params, message, client):
        ...
        try:
            embed_msg = Embed(title='Command Line Output', color=0x00ff00)
            embed_msg.add_field(name="\u200b", value=stdout, inline=False)

            embed_msg.set_footer(text=user_name, icon_url=member_object.avatar_url)
            await message.channel.send(embed=embed_msg)
        except Exception:
            await message.channel.send(
                "Sorry, the response is too large for an embed. Use RawServerCommand instead :)")
            return
